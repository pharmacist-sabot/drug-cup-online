    ...
    <div class="card report-options no-print">
      ...
      <div class="form-group">
        ...
        <select v-model="selectedPcu" :disabled="!processedData.length">
          ...
          <option v-for="pcu in pcuList" :key="pcu.id" :value="pcu.id">{{ pcu.name }}</option>
        </select>
      </div>
    </div>
      ...
      <div class="table-wrapper">
        <table>
          <thead>
            <tr>
              <th>รายการ</th>
              <th class="text-center" v-if="selectedPcu === 'all'">ยอดรวม (ทุก รพ.สต.)</th>
              <th class="text-center" v-else>ยอดเบิก ({{ getPcuName(selectedPcu) }})</th>
              <th class="text-center" v-if="selectedPcu === 'all'" v-for="pcu in pcuList" :key="pcu.id">{{ pcu.name }}</th>
            </tr>
          </thead>
          <tbody>
            <tr v-for="item in processedData" :key="item.item_id">
              <td>{{ item.item_name }}</td>
              <td class="text-center bold">{{ getDisplayQuantity(item) }}</td>
              <td class="text-center" v-if="selectedPcu === 'all'" v-for="pcu in pcuList" :key="pcu.id">
