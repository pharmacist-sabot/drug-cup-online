<template>
  <div class="container">
    <h2><i class="fas fa-file-signature"></i> สร้างบันทึกข้อความขออนุมัติเบิกจ่าย</h2>
    <p class="text-muted">เลือกรอบเบิกเพื่อสร้างเอกสารบันทึกข้อความเสนอผู้อำนวยการ</p>

    <div class="card report-options no-print">
      <div class="form-group">
        <label for="report-period">1. เลือกรอบเบิก</label>
        <select id="report-period" v-model="selectedPeriod" @change="fetchReportData">
      ...
      <div class="form-group action-buttons-group">
        <label>&nbsp;</label>
        <button @click="printMemo" :disabled="!selectedPeriod || reportData.length === 0" class="btn btn-primary">
...
onMounted(async () => {
  const { data } = await supabase.from('requisition_periods_drugcupsabot').select('id, name').order('start_date', { ascending: false });
  periods.value = data || [];
});
...
.action-buttons-group { flex-grow: 0; }
</style>