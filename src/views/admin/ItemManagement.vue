  ...
  <div class="container">
        ...
        <table>
          ...
          <tbody>
            <tr v-for="item in filteredItems" :key="item.id">
              <td><input type="text" v-model="item.name" class="form-control-table"></td>
              <td><input type="number" step="0.01" v-model.number="item.price_per_unit" class="form-control-table text-right"></td>
              <td><input type="text" v-model="item.unit_pack" class="form-control-table"></td>
              <td><input type="text" v-model="item.notes" class="form-control-table"></td>
              <td class="text-center">
                <label class="switch">
                  <input type="checkbox" v-model="item.is_available">
    ...
    <div v-if="showAddItemModal" class="modal-overlay" @click.self="showAddItemModal = false">
      <div class="modal-content card">
        <h3><i class="fas fa-plus-circle"></i> เพิ่มรายการยา/เวชภัณฑ์ใหม่</h3>
        <form @submit.prevent="handleAddItem">
          <div class="form-group">
            <label for="name">ชื่อรายการ <span class="required">*</span></label>
            <input type="text" id="name" v-model="newItem.name" required>
          </div>
          <div class="form-grid">
            <div class="form-group">
              <label for="price">ราคาต่อหน่วย <span class="required">*</span></label>
              <input type="number" id="price" step="0.01" min="0" v-model.number="newItem.price_per_unit" required>
            </div>
            <div class="form-group">
              <label for="unit_pack">หน่วยนับ</label>
              <input type="text" id="unit_pack" v-model="newItem.unit_pack">
            </div>
          </div>
          <div class="form-group">
...
async function handleAddItem() {
  ...
  try {
    ...
    if (existingCategory) {
      ...
      const maxCategoryOrder = items.value.reduce((max, item) => Math.max(max, item.category_order || 0), 0);
